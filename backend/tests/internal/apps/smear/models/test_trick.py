...
def test_update_if_out_of_cards_did_not_follow_trump():
    ...
    all_plays.append(lead_play)
    play = Play.objects.create(trick=trick, card="2S", player=p2)
    all_plays.append(play)

    trick.hand.update_if_out_of_cards(p2, Card(representation="2S"), lead_play, all_plays)
...
def test_is_card_invalid_to_play_allows_non_jick_if_jick_suit_is_lead():
    ...
    lead_play = Play.objects.create(trick=trick, card="2D", player=p1)

    assert trick.is_card_invalid_to_play(Card(representation="2S"), p2, lead_play) is None
...
def test_is_card_invalid_to_play_allows_non_jick_suit_if_jick_is_lead():
    ...
    lead_play = Play.objects.create(trick=trick, card="JH", player=p1)
